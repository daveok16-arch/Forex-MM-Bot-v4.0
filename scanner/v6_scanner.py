...
class V6Scanner:
    """Next-gen scanner with ONNX-only inference, order flow, sentiment, calendar."""
    ...
    def _load_ensemble(self, mdir):
        ...
        print(f"[V6] Ensemble loaded: {load_path}")
        ...
        class OrderFlow:
            ...
            def analyze(self):
                ...
                if total_bid > total_ask * 1.5:
                    profile = "bid_dominant"
                elif total_ask > total_bid * 1.5:
                    profile = "ask_dominant"
                else:
                    profile = "neutral"
                
                if recent_delta > 0 and strength > 0.3:
                    signal = "BUY"
                elif recent_delta < 0 and strength > 0.3:
                    signal = "SELL"
                else:
                    signal = "HOLD"
                ...
                return {
                    ...
                    "signal": signal,
                    "profile": profile,
                    "bid_ratio": total_bid / (total_bid + total_ask + 1e-8)
                }
        return OrderFlow()
        ...
        class Sentiment:
            def get_sentiment(self, pair):
                ...
                return {
                    ...
                    "bullish_pct": 50 + int(sentiment * 50),
                    "bearish_pct": 50 - int(sentiment * 50),
                    "source": "social_mock",
                    "trending_topics": ["forex", pair.lower(), "trading"]
                }
        return Sentiment()
    ...
    def _init_calendar(self):
        ...
        class Calendar:
            def fetch_events(self):
                ...
                now = datetime.utcnow()
                events = [
                    {"time": (now + timedelta(hours=2)).strftime("%Y-%m-%d %H:%M"), "currency": "USD", "event": "NFP", "impact": "High", "forecast": "200K", "previous": "180K"},
                    {"time": (now + timedelta(hours=4)).strftime("%Y-%m-%d %H:%M"), "currency": "EUR", "event": "ECB Rate", "impact": "High", "forecast": "4.50%", "previous": "4.50%"}
                ]
                return events
            ...
            def get_alert(self, pair):
                ...
                if not relevant:
                    return ""
                alert = "⚠️ <b>UPCOMING EVENTS:</b>\n"
                for e in relevant:
                    alert += f"• {e['time']} - {e['event']} ({e['currency']}) - {e['impact']}\n"
                return alert
        return Calendar()
    
    async def _fetch_data(self, pairs: List[str], batch_size: int = 3) -> Dict[str, Dict]:
        data = {}
    ...
    def _composite_score(self, pair: str, data: Dict) -> Tuple[float, str, Dict]:
        ...
        # Skip calendar API - use defaults
        ...
        composite = (hyper_score * weights["hyper"] + 
        ...
        for pair, d in data.items():
            ...
            if conf >= 75 and sig != "HOLD":
                ...
                sizing = self.position_sizer.calculate(
                    d["current_price"],
                    d["current_price"] * 0.98 if sig == "BUY" else d["current_price"] * 1.02,
                    10000.0,
                    vol
                )
    ...
    async def _emit_signal(self, sig: Dict):
        ...
        components = (
            ...
            f"Hypernetwork: {d['hyper']:+.3f}\n"
            f"Order Flow: {d['orderflow']:+.3f} ({d['orderflow_signal']})\n"
            f"Sentiment: {d['sentiment']:+.3f}\n"
            f"Fundamental: {d['fundamental']:+.3f}\n"
            f"Regime: {d['regime']}"
        )
        
        sent = d['sentiment_data']
        sentiment_info = (
            f"📱 <b>Social Sentiment:</b>\n"
            f"Score: {sent['score']:+.2f}\n"
            f"Bullish: {sent['bullish_pct']}% | Bearish: {sent['bearish_pct']}%\n"
            f"Topics: {', '.join(sent['trending_topics'][:3])}"
        )
        
        sizing_info = (
            f"📊 <b>Position Size:</b> {sizing['position_size']}\n"
            f"💰 Risk: ${sizing['risk_amount']}\n"
            f"📏 Stop: {sizing['stop_distance']:.5f}\n"
            f"⚖️ Vol Adj: {sizing['vol_adjustment']}"
        )
        
        calendar_info = sig.get("calendar", "")
        
        msg = (f"🚀 <b>V6 SIGNAL: {pair} {signal}</b>\n"
               f"Price: {price}\n"
               ...
               f"{sizing_info}\n\n"
               f"{calendar_info}\n\n"
               f"⏱ Act within 3 minutes")
        
        print(f"[V6 SIGNAL] {pair} {signal} {conf:.1f}%")
        await self.tg._send(msg)
    
    async def _send_heartbeat(self):
        stats = (f"💓 <b>V6 Heartbeat</b>\n"
                 f"Scans: {self.scan_count}\n"
                 f"Pairs tracked: {len(self.price_history)}\n"
                 f"Active signals: {len(self.active_signals)}\n"
                 f"Models: Hypernetwork + OrderFlow + Sentiment + Calendar")
    ...
    async def run(self, interval=300):
        ...
        await self.tg._send("🚀 <b>V6 Scanner v6.0</b> AI-powered trading signals")
